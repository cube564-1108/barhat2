...
def shrink(data: bytes, max_side: int, quality: int) -> tuple[bytes, str, str]:
    ...
    try:
        ...
        if has_alpha:
            ...
            return buf.getvalue(), "png", f"{img.size[0]}x{img.size[1]}"
        img.convert("RGB").save(buf, "JPEG", quality=quality, optimize=True)
        return buf.getvalue(), "jpeg", f"{img.size[0]}x{img.size[1]}"
...
def main() -> int:
    ...
    slug = re.sub(r"[^a-z0-9]+", "-", stem.lower()).strip("-") or "img"
