...
def test_bulk_import():
    ...
    # Повторная загрузка НЕТРОНУТОГО файла: в выгрузке у неразмеченного товара
    # роль пуста, и наивное «пустая роль = снять» рапортовало бы «снято: N» на
    # строках, которых человек не касался.
    again = storage.set_time_norms_bulk([
        {"offer_id": 400, "role": ""},   # норма снята предыдущей загрузкой
        {"offer_id": 999, "role": ""},   # такого товара в нормах не было вовсе
    ], "tester")
    check("повторная загрузка не снимает то, чего не было",
          again["cleared"] == 0, f"получено {again}")

    # А вот реально размеченный товар пустая роль снимает — это способ отменить
    # разметку прямо в файле, и он обязан работать.
    storage.set_time_norm("offer", 300, role="berry", username="tester")
    removed = storage.set_time_norms_bulk([{"offer_id": 300, "role": ""}], "tester")
    check("пустая роль снимает существующую норму", removed["cleared"] == 1,
          f"получено {removed}")
