...
# 22.09.2026 правило переехало в общий `bySites`: раньше здесь считались два
# вхождения одной строки, и добавление третьего потребителя (секции вкладки
# «Мои») эту проверку сломало бы, хотя код стал лучше. Проверяем не число
# копий, а что все потребители ходят через одну функцию. Поведение —
# в scripts/test_courier_mine_sections.js (разделы 1 и 3).
check("пустой выбор салонов = все салоны",
      "if (!state.sites.length) return true;" in js
      and "function bySites(" in js,
      "(правило живёт в одном месте)")
check("фильтр салонов применяется и к ленте, и к счётчикам, и к секциям",
      js.count("bySites(") >= 4,
      "(объявление плюс visibleOrders, counts и mineGroups)")
