...
# --- фильтр по салонам ------------------------------------------------------
# Проверки по исходнику, а не по поведению: DOM тут не поднять. Ловят они
# ровно те три вещи, которые ломаются молча и выглядят как «приложение врёт».

# Пустой выбор обязан означать «все салоны», а не «ни одного»: иначе после
# сброса фильтра экран пустеет, и понять это невозможно.
check("пустой выбор салонов = все салоны",
      js.count("if (!state.sites.length) return true;") >= 2,
      "(правило нужно и в отборе ленты, и в счётчиках табов)")
check("выбор нескольких салонов, а не одного",
      "state.sites.indexOf" in js and "state.sites.push" in js,
      "(множественный выбор: за ходку берут заказы из разных салонов)")
check("выбор переживает перезапуск",
      "localStorage" in js and "courier-app:sites" in js)
check("хранилище недоступно — приложение не падает",
      js.count("catch (e)") >= 2, "(localStorage бросает в приватном режиме)")
check("счётчики табов учитывают выбранные салоны",
      "function counts()" in js and js.index("function counts()") < js.index("var scope"),
      "")
# Салон — не граница приватности внутри города: данные уже пришли одним
# запросом, и ходить за ними снова незачем.
sites_block = js[js.index("function toggleSite"):js.index("// === Отбор и порядок")]
...
# Забирать несобранный заказ нельзя (решение владельца 2026-09-11). Кнопка
# при этом обязана остаться видимой и объяснять, чего ждём: исчезнувшее
# действие читается как поломка приложения, а живая кнопка, отвечающая
# отказом, — как издевательство.
check("кнопка забора у несобранного заказа заблокирована",
      "order.is_ready" in js_code and "Ждём отметки" in js_code,
      "(иначе курьер жмёт и получает отказ)")
check("подпись объясняет, чего ждать", "cd-note" in js_code and ".cd-note" in css)
check("обход запрета не остался в коде", "force_not_ready" not in js_code,
      "(подтверждение «всё равно забираю» отменено)")

# Действие человека не должно теряться из-за фонового запроса. 16.09.2026
# переключение дня, попавшее в момент автообновления (раз в 30 секунд),
# молча отбрасывалось: дата в шапке менялась, лента оставалась старой до
# следующего тика — снаружи это выглядело как зависание на полминуты.
check("нажатие во время загрузки не теряется",
      bool(re.search(r"function loadFeed\(force\)[\s\S]{0,200}"
                     r"if \(state\.loading\)[\s\S]{0,120}reloadWanted = true", js_code)),
      "(занятая лента обязана запомнить запрос человека, а не отбросить его)")
check("отложенный запрос действительно выполняется",
      bool(re.search(r"state\.reloadWanted = false;[\s\S]{0,80}loadFeed\(true\)", js_code)),
      "(запомнить мало — надо повторить после текущего)")
check("смена дня просит ленту принудительно",
      re.search(r"function setDate[\s\S]{0,400}loadFeed\(true\)", js_code),
      "(иначе действие отбрасывается при занятой ленте)")
check("кнопка «обновить» тоже не отбрасывается",
      not re.search(r"el\.refresh\.addEventListener[\s\S]{0,200}if \(state\.loading\) return",
                    js_code),
      "(кнопка, которая иногда не работает, хуже медленной)")
check("ответ на прошлый день не затирает выбранный",
      "answered !== state.date" in js_code,
      "(на медленной сети ответы приходят не по порядку)")
