...
// Заглушка DOM общая для всех сторожей интерфейса — см. scripts/lib/dom_stub.js
const { makeSandbox: createSandbox, makeEl, flush } = require('./lib/dom_stub.js');

const SRC = path.join(__dirname, '..', 'src', 'dashboard', 'writeoffs.js');

/** Песочница с модулем списаний: путь к файлу здесь один на весь прогон. */
function makeSandbox(fetchImpl, xhrImpl) {
    return createSandbox({ src: SRC, fetch: fetchImpl, xhr: xhrImpl });
}
