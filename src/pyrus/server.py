...
def health_check():
    ...
    **Дешёвая по умолчанию, подробная по запросу.** Замер 2026-09-08: ручка
    ...
    внешний мониторинг превращает этот риск в аварию.
        ...
        GET /health         — жив ли процесс и на месте ли данные. **Ни одного
                              обращения к базе**: только файловая система.
        GET /health?full=1  — вся диагностика: проба записи, остаток квоты
                              ПланФакта, состояние витрин, инварианты схемы.

    Почему проба записи не в дешёвом ответе, хотя это она отличает живой сайт
    от сайта с зависшим входом. Замер прода 09.09.2026 после первой правки:
    файловые блоки — 0–8 мс, а два оставшихся соединения (проба записи и
    чтение квоты) — 103–386 и 113–645 мс, то есть **вся** цена ручки. Пока они
    в дешёвом ответе, «дешёвого ответа» нет.

    Мониторингу это ничего не ломает: опасны были не отдельные секунды, а то,
    что столько стоил КАЖДЫЙ вызов — два одновременных занимали оба воркера.
    Внешняя проверка «база принимает запись» ходит на `?full=1` раз в минуту и
    стоит около секунды, что безобидно.
