...
# Итог последней ПРОБНОЙ отправки: её делает человек руками, и её ответ
# нужен позже, когда экран с сообщением уже закрыт.
PUSH_TEST_KEY = "push_last_test"

# Журнал последних отправок: что, когда, скольким и чем кончилось.
# Отметка о прогоне говорит «упал ли тик», а этот журнал — «ушло ли
# уведомление по конкретному заказу»; между тиками иначе ничего не
# остаётся, и отправка двадцатиминутной давности невидима.
PUSH_LOG_KEY = "push_send_log"
