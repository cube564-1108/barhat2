...
from .storage import CARD_KINDS  # виды заявок на рабочих картах
...
def mark_paid(invoice_id):
    ...
    # Трата с рабочей карты отмечается оплаченной наравне со счётом (решение
    # владельца 21.09.2026). Раньше здесь стоял отказ: считалось, что её
    # терминальное состояние — «Согласован» плюс признак разноски. На практике
    # такие заявки навсегда оставались в «Согласован» и смешивались с теми,
    # что правда ждут действия. Обычно статус переводит сама разноска
    # (cards_sync._settle_card_expense), эта кнопка — для случаев мимо неё.
...
# Разноска оплаченных счетов по данным ПланФакта живёт в отдельном модуле
# (фаза 0 плана 2026-09-15): объединить её с разноской заявок по картам,
# пока она лежала здесь, было нельзя — этот файл сам импортирует cards_sync.
# Имена реэкспортируются: на них ссылается scripts/test_planfact_sync.py.
from .planfact_sync import (  # noqa: E402
