...
def preview_attachment(attachment_id):
    ...
    # дотянуться. У PDF при этом нужен `allow-scripts`: пустой `sandbox` гасит
    # встроенный просмотрщик, и вместо счёта человек видит белый прямоугольник.
    #
    ...
    response.headers["Content-Security-Policy"] = (
        "sandbox allow-scripts" if content_type == "application/pdf" else "sandbox"
    )
