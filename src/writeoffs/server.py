        ...
        for position, href in zip(targets, hrefs):
            ...
            if result is None:
                errors.append({"document": doc.get("name"), "position": name,
                               "error": _explain_moysklad_error(client.last_error)})
                continue
            updated_positions += 1
            doc_touched = True
...
# Код МойСклада для «нарушено ограничение уникальности параметра»
MS_ERROR_DUPLICATE_NAME = 3006


def _explain_moysklad_error(last_error) -> str:
    """
    Причина отказа словами, по которым понятно, что делать.

    «МойСклад отклонил правку» — это не объяснение: человек не знает, чинить
    ему что-то или повторить позже. Текст отказа МойСклад называет сам, а
    известные коды переводим в действие.

    3006 по документу списания означает задвоенный номер: при одновременном
    создании нескольких документов МойСклад присвоил им один и тот же. Пока
    номера совпадают, документ нельзя сохранить вообще — ни нами, ни руками.
    Так 04.09.2026 три заявки (#42, #44, #45), согласованные в одну секунду,
    получили номер 00210-00061 и остались нулевыми.
    """
    if not last_error:
        return "МойСклад не ответил — попробуйте позже"

    text = last_error.get("text") or "причина не названа"
    code = last_error.get("code")

    if code == MS_ERROR_DUPLICATE_NAME:
        return ("у документа задвоен номер — такой же есть у другого списания. "
                "Переименуйте документы в МойСкладе, тогда цена проставится")
    return f"{text}" + (f" (код {code})" if code else "")
