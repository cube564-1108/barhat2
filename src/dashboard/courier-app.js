...
(function () {
    ...
    // Пауза после таймаута, прежде чем пробовать снова.
    //
    // БЕЗ НЕЁ ТАЙМАУТ ДЕЛАЕТ СЕРВЕРУ ХУЖЕ, а не лучше. Прерывание на клиенте
    // не останавливает обработчик: он доработает свои полторы минуты и займёт
    // поток воркера до конца. Раньше следующий запрос уходил, только когда
    // предыдущий ОТВЕТИЛ, то есть один в ~100 секунд. Освободив клиент на 45-й
    // секунде, мы бы отправляли следующий уже через 45–75 секунд — и держали
    // бы на сервере не один обработчик на курьера, а полтора-два.
    //
    // Воркеров два по восемь потоков. Ровно так вчера и клали сайт: правка,
    // разумная с одной стороны, добавляла нагрузку с другой.
    //
    // 45 + 60 = 105 секунд — не чаще, чем лента отвечала в худших замерах
    // (53–113 с, лог 16.09.2026). То есть нагрузка заведомо не растёт.
    var TIMEOUT_BACKOFF_MS = 60000;
    ...
    var state = {
        ...
        retryNotBefore: 0,  // до этого времени не просим сервер снова
        loading: false,
        openOrderId: null,
    ...
    function toast(message, kind, ms) {
        if (window.BarhatUI) window.BarhatUI.toast(message, kind || 'info', ms);
    }
    ...
    function apiGet(url, timeoutMs) {
        ...
        return fetch(url, options).then(function (response) {
            ...
            if (error && error.name === 'AbortError') {
                var timedOut = new Error('сервер не ответил за '
                    + Math.round((timeoutMs || REQUEST_TIMEOUT_MS) / 1000) + ' с');
                // Признак полем, а не разбором текста: текст меняют при первой
                // же правке формулировки, и отсрочка молча перестанет
                // применяться — то есть вернётся ровно та лишняя нагрузка,
     ...
     * Перечитать ленту.
     *
     * manual=true — нажали «Обновить». Отличать нужно ради одного: молча
     * ничего не сделать в ответ на нажатие нельзя, кнопка выглядит сломанной.
     * Автоматический тик, наоборот, объясняться не обязан.
     */
    function loadFeed(manual) {
        if (state.loading) return Promise.resolve();

        // Отсрочка после таймаута — см. TIMEOUT_BACKOFF_MS. Держит обещание
        // «запросов на сервер не станет больше, чем было до таймаута».
        var wait = (state.retryNotBefore || 0) - Date.now();
        if (wait > 0) {
            if (manual) {
                toast('Сервер не успевает ответить. Повторим через '
                    + Math.ceil(wait / 1000) + ' с');
            }
        ...
        return apiGet(url).then(function (payload) {
            ...
            state.retryNotBefore = 0;   // ответили — отсрочка больше не нужна
            ...
            // Сервер не успел — значит он сейчас занят, и долбить его чаще,
            // чем он отвечает, нельзя: обработчик прошлого запроса всё ещё
            // держит поток воркера.
            if (error && error.timedOut) {
                state.retryNotBefore = Date.now() + TIMEOUT_BACKOFF_MS;
            }
    ...
    function siteOptions() {
        // На «Моих» счётчики салонов считают то же, что и счётчик на табе, —
        // незакрытые дела. Иначе «Мои 0» и «Свердловский 3» в слое выбора
        // салонов спорят друг с другом на одном экране.
        var byTab = state.filter === 'mine'
            ? state.orders.filter(isMineOpen)
            : ordersByTab();
        var map = {};
    ...
    function mineFeedHtml() {
        ...
        if (!parts.length) {
            ...
            //
            // Но только если дел нет НА САМОМ ДЕЛЕ. При включённом фильтре по
            // салонам активные заказы бывают просто скрыты — сказать в этот
            // момент «всё доставлено» значит отпустить курьера домой с тремя
            // несобранными букетами. Считаем без фильтра: он свой же и снимет.
            //
            // И без слова «сегодня»: лента показывает выбранный день, а он
            // бывает любым.
            var openAnywhere = state.orders.some(isMineOpen);
            parts.push('<p class="cd-empty">'
                + esc(done && !openAnywhere
                      ? 'Все заказы за этот день доставлены.'
                      : emptyText())
                + '</p>');
        }
    ...
    /**
     * Строка адреса для карт.
     *
     * Город впереди нужен тем адресам, что приходят из CRM одной улицей
     * («ул. Ленина, 45») — без него Яндекс ищет её по всей стране. Но в части
     * городов CRM пишет адрес уже с регионом и городом («Свердловская область,
     * Екатеринбург, ул. Бажова, 89»), и второй «Екатеринбург» впереди лишний.
     *
     * Ищем город ЦЕЛЫМ СЛОВОМ внутри части адреса, а не подстрокой: «Новосибирск»
     * входит подстрокой в «Новосибирская область», и поиск подстрокой посчитал бы
     * город уже названным там, где названа только область — то есть ровно в том
     * случае, ради которого подстановка и нужна. Знаки препинания и дефисы при
     * сравнении становятся пробелами с обеих сторон, поэтому «г. Екатеринбург»
     * и «Ростов-на-Дону» узнаются как есть.
     */
    function routeQuery(order) {
        ...
        function words(value) {
            return ' ' + String(value).toLowerCase().replace(/[^0-9a-zа-яё]+/gi, ' ').trim() + ' ';
        }

        var needle = words(city);
        var parts = address.split(',');
        for (var i = 0; i < parts.length; i++) {
            if (words(parts[i]).indexOf(needle) !== -1) return address;
        }
        return city + ', ' + address;
    }
    ...
    /**
     * Что сказать сразу после включения.
     *
     * Сервер шлёт пробное уведомление той же подпиской. Это единственный
     * честный ответ на «а они работают»: настоящий пуш уходит, только когда
     * в городе появится новый свободный заказ, и в пустой день молчание
     * неотличимо от поломки (разбор 17.09.2026).
     *
     * Три исхода — три разных действия человека, поэтому и текста три.
     */
    function announceSubscribed(data) {
        if (data.test_sent) {
            toast('Уведомления включены. Сейчас придёт пробное — если его нет, '
                + 'проверьте разрешения в настройках телефона', 'success');
        } else if (data.test_reason === 'not_configured') {
            toast('Подписка сохранена, но на сервере не заведены ключи уведомлений. '
                + 'Сообщать о заказах пока нечем — скажите администратору', 'error');
        } else if (data.test_reason === 'bad_key') {
            toast('Подписка сохранена, но сервер не может прочитать свой ключ '
                + 'уведомлений — отправить их нельзя. Это чинит администратор, '
                + 'телефон ни при чём', 'error', 20000);
        } else if (data.test_reason === 'no_library') {
            toast('Подписка сохранена, но на сервере не установлена библиотека '
                + 'отправки уведомлений. Это чинит администратор — телефон ни при чём',
                'error');
        } else if (data.test_reason === 'rejected' || data.test_reason === 'expired') {
            // Отказал push-сервис, а не телефон. Его текст показываем как есть:
            // гонять человека по настройкам телефона здесь бессмысленно.
            toast('Служба уведомлений отклонила отправку'
                + (data.test_status ? ' (код ' + data.test_status + ')' : '') + '. '
                + (data.test_detail || 'Причина не названа')
                + ' — покажите это администратору', 'error', 20000);
        } else {
            toast('Подписка сохранена, но пробное уведомление не дошло'
                + (data.test_detail ? ': ' + data.test_detail : '. ')
                + ' Проверьте разрешения уведомлений в настройках телефона '
                + 'и режим экономии заряда', 'error', 20000);
        }

        // Подписка есть, а адресатом человек не является: уведомления о новых
        // заказах выбирают АКТИВНЫЙ ПРОФИЛЬ КУРЬЕРА В ГОРОДЕ ЗАКАЗА. У
        // администратора и управляющего профиля обычно нет, и пробное к ним
        // придёт, а настоящее — никогда. Это разные вещи, и путать их нельзя.
        if (data.is_recipient === false) {
            toast('Пробное придёт, а сообщения о новых заказах — нет: '
                + 'у вашей учётной записи нет профиля курьера с городом. '
                + 'Его заводит администратор в разделе «Контроль доставки»', 'error');
        }
    }
    ...
    function subscribePush() {
        ...
        Notification.requestPermission().then(function (permission) {
            ...
            }).then(sendSubscription).then(function (payload) {
                state.pushOn = true;
                renderPushButton();
                var data = (payload && payload.data) || {};
                // Город приходит вместе с ответом: если профиля курьера нет,
                // предупреждение «вы не адресат» должно появиться сразу, а не
                // после следующей загрузки профиля
                if (data.city) state.city = data.city;
                render();
                announceSubscribed(data);
            });
        }).catch(function (error) {
